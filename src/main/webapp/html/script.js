...
const RACE_ARRAY = ['HUMAN', 'DWARF', 'ELF', 'GIANT', 'OPC', 'TROLL', 'HOBBIT'];
const PROFESSION_ARRAY = ['WARRIOR', 'ROUGE', 'SORCERER', 'CLERIC', 'PALADIN', 'NAZGUL', 'WARLOCK', 'DRUID'];
...
function createSelect(optionsArray, defaultValue) {
    //return `<select> ${createSelectOption(optionsArray,defaultValue)} </select>`
...
function onAccountsPerPagesChangeHandler(e) {
    //const newAccountsPerPageValue = e.currentTarget.value;
    //accountsPerPage = newAccountsPerPageValue;
...
function createAccountPerPageDropDown(){
    ...
    $dropDown.addEventListener('change', onAccountsPerPagesChangeHandler)
...
function setActiveButton(buttonIndex = 0){
    ...
    const $currenActiveButton = Array.from($buttonsContainer.children)[currentPageNumber];
...
function editAccountHandler(e){
    ...
    //const currentRowCalls = currentRow.querySelectorAll('.cell');
    ...
    //currentRow.querySelector('[data-account-level]')
    //currentRow.querySelector('[data-account-birthday]')
